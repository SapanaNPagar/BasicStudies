...
public class post_SD {
	...
	public void user_hit_the_post_request_from_String() {
		resp = RestAssured.given().accept(ContentType.JSON).relaxedHTTPSValidation().when()
				.body("{    \"name\": \"Tom\",    \"job\":\"Actor\"}").post("https://reqres.in/api/users");
		resp.then().log().all();
